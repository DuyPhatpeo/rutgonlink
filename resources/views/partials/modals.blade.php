...
<div id="loginModal" onclick="if(event.target===this) Modal.close('loginModal')" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden animate-in fade-in duration-300">
    <div class="bg-white rounded-[40px] p-8 md:p-12 w-full max-w-md shadow-2xl relative">
        ...
        <form onsubmit="Auth.handleLogin(event)" class="space-y-4 md:space-y-6">
            ...
            <button type="button" onclick="Auth.googleLogin()" class="w-full bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 font-black py-3.5 md:py-4 rounded-2xl transition-all flex items-center justify-center gap-3 shadow-sm active:scale-95 text-xs md:text-sm uppercase tracking-widest">
